...
final class RoutesEndpointTests {
    ...
    @Test("test RoutesEndpoint.stops with filters")
    func stopsWithFilters() {
        let endpoint = RoutesEndpoint.stops(
            routeName: "72",
        ...
        #expect((request.parameters ?? []).contains(HTTPParameter(key: "destination", value: "Downtown Berkeley")))
        #expect((request.parameters ?? []).contains(HTTPParameter(key: "scheduleType", value: "Saturday")))
        #expect((request.parameters ?? []).contains(HTTPParameter(key: "byPattern", value: "true")))
    }
    ...
    @Test("test RoutesEndpoint.tripsInstructions with direction")
    func tripsInstructionsWithDirection() {
        let endpoint = RoutesEndpoint.tripsInstructions(routeName: "72", direction: "Southbound", scheduleType: .weekday)
        let request = endpoint.getRequest(token: Constants.mockToken)
    ...
    @Test("test RoutesEndpoint.trips with direction")
    func tripsWithDirection() {
        let endpoint = RoutesEndpoint.trips(routeName: "72", direction: "Northbound")
        let request = endpoint.getRequest(token: Constants.mockToken)
    ...
    func scheduleWithFilters() {
        let endpoint = RoutesEndpoint.schedule(
            routes: "72",
            direction: "Northbound",
            destination: "Downtown Berkeley",
            dayCode: "Weekday",
    ...
    func timetable() {
        ...
        #expect(endpoint.path == "/route/72/timetable/Southbound")
        #expect(request.httpMethod == .GET)
        #expect(request.baseUrl == "https://api.actransit.org/transit/route/72/timetable/Southbound")
        #expect((request.parameters ?? []).contains(HTTPParameter(key: Constants.tokenKey, value: Constants.mockToken)))
    }

    @Test("test RoutesEndpoint.timetable without direction")
    func timetableWithoutDirection() {
        let endpoint = RoutesEndpoint.timetable(routes: "72")
        #expect(endpoint.path == "/route/72/timetable")
    }

    @Test("test RoutesEndpoint.timetable with dayCode and hasAllStops")
    func timetableWithOptions() {
        let endpoint = RoutesEndpoint.timetable(routes: "72", dayCode: "Saturday", hasAllStops: true)
        let request = endpoint.getRequest(token: Constants.mockToken)

        #expect((request.parameters ?? []).contains(HTTPParameter(key: "dayCode", value: "Saturday")))
        #expect((request.parameters ?? []).contains(HTTPParameter(key: "hasAllStops", value: "true")))
    }

    @Test("test RoutesEndpoint.tripStopsToday")
    func tripStopsToday() {
        let endpoint = RoutesEndpoint.tripStopsToday(routes: "72")
        let request = endpoint.getRequest(token: Constants.mockToken)

        #expect(endpoint.path == "/route/72/tripstops")
        #expect(request.httpMethod == .GET)
        #expect(request.baseUrl == "https://api.actransit.org/transit/route/72/tripstops")
        #expect((request.parameters ?? []).contains(HTTPParameter(key: Constants.tokenKey, value: Constants.mockToken)))
    }

    @Test("test RoutesEndpoint.tripStopsToday with direction")
    func tripStopsTodayWithDirection() {
        let endpoint = RoutesEndpoint.tripStopsToday(routes: "72", direction: "Southbound")
        let request = endpoint.getRequest(token: Constants.mockToken)
    ...
    @Test("test RoutesEndpoint.waypointsFast with scheduleType")
    func waypointsFastWithScheduleType() {
        let endpoint = RoutesEndpoint.waypointsFast(routes: "72", scheduleType: .weekday)
        let request = endpoint.getRequest(token: Constants.mockToken)
    ...
    @Test("test RoutesEndpoint.destinations")
    func destinations() {
        let endpoint = RoutesEndpoint.destinations(routeName: "72")
        let request = endpoint.getRequest(token: Constants.mockToken)

        #expect(endpoint.path == "/route/72/destinations")
        #expect(request.httpMethod == .GET)
        #expect(request.baseUrl == "https://api.actransit.org/transit/route/72/destinations")
        #expect(request.parameters == [HTTPParameter(key: Constants.tokenKey, value: Constants.mockToken)])
    }

    @Test("test RoutesEndpoint.exceptions")
    func exceptions() {
        let endpoint = RoutesEndpoint.exceptions(routes: "72")
        let request = endpoint.getRequest(token: Constants.mockToken)

        #expect(endpoint.path == "/route/72/exceptions")
        #expect(request.httpMethod == .GET)
        #expect(request.baseUrl == "https://api.actransit.org/transit/route/72/exceptions")
        #expect(request.parameters == [HTTPParameter(key: Constants.tokenKey, value: Constants.mockToken)])
    }

    @Test("test RoutesEndpoint.exceptions with booking")
    func exceptionsWithBooking() {
        let endpoint = RoutesEndpoint.exceptions(routes: "72", booking: "Current")
        let request = endpoint.getRequest(token: Constants.mockToken)
